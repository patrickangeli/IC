...
def main(file_input_cota: str, 
    ...
    for _ in range(num_camadas_lstm_empilhadas):
        model.add(LSTM(units=num_celulas_lstm, 
                    return_sequences=True, input_shape=(x_train_redimension.shape[1], num_features), dropout=0.2))
    ...
    dir_result = f"../Entrada/novo/Saida/{file_input_cota.split('/')[1]}/LSTM/80_85_90/{tempo_antecedencia}hours_dropout"
    Path(dir_result).mkdir(exist_ok=True, parents=True)
    file_output = f"resultTrain_{num_celulas_lstm}neurons_{num_steps}_steps_{batch_size}_batch_dropout.txt"
    ...
    file_output = f"resultTest_{num_celulas_lstm}neurons_{num_steps}_steps_{batch_size}_batch_dropout.csv"
    filename_output = f"{dir_result}/{file_output}"
