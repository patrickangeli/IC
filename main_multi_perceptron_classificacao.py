...
def main(file_input_cota: str, 
    ...
    X, Y = create_sequences(data_X, data_Y, tempo_antecedencia, lst_datetimes)
...
def train_neural_network(x_train, y_train, num_neurons, num_epochs, func_camada_oculta, learning_rate, batch_size, func_camada_saida, dropout):
    num_classes = 3
    y_train_encoded = to_categorical(y_train, num_classes=num_classes)
    x_train = np.expand_dims(x_train, axis=1)
    model = Sequential()
    model.add(SimpleRNN(units=num_neurons, activation=func_camada_oculta, dropout=dropout))
...
def weighted_binary_crossentropy(y_true, y_pred, weight=6.0):
