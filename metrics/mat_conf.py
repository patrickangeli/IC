...
def confusionMatrixPlot(entrada_dir):

    #ler entrada em um df
    df = pd.read_csv(entrada_dir)
    y_true = df['y_true']
    y_pred = df['y_pred']
    labels = ["NORMAL", "ATENCAO", "INUNDACAO"]
    # print(df.head())
...
if __name__ == '__main__':
    entrada_dir = 'Saida/3hours/relu/resultTest_24neurons_500epochs.csv'
    confusionMatrixPlot(entrada_dir)